...
namespace WoW_mod
{
    ...
    {
        private readonly WoW_mod _plugin;
        private readonly Config _config;

        public EventSystem(WoW_mod plugin, Config config)
        {
            _plugin = plugin;
            _config = config;
        }

        public void Initialize()                                                                                //  SPELL A INTEGRER A FAIRE JUSQUA la fin !!!!!!
        {
        ...
        private HookResult PlayerSpawnHandler(EventPlayerSpawn @event, GameEventInfo _)
        {
            ...
            if (race != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    WoW_mod.Instance.EffectManager.ClearEffects(player);
        ...
        private HookResult PlayerDeathHandler(EventPlayerDeath @event, GameEventInfo _)
        {
            ...
            WoW_mod.Instance.EffectManager.ClearEffects(victim);

            return HookResult.Continue;
        }
                                                                                                                                                                                            // FIN DE SPELL A RENTER POUR TOUT LES SPELL POSSIBLE A FAIRE
